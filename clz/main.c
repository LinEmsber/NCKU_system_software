...
static inline __attribute__((always_inline)) void get_cycles(unsigned *high, unsigned *low)
{
	asm volatile ("CPUID\n\t"
			"RDTSC\n\t"
			"mov %%edx, %0\n\t"
...
static inline __attribute__((always_inline)) void get_cycles_end(unsigned *high, unsigned *low)
{
	asm volatile("RDTSCP\n\t"
			"mov %%edx, %0\n\t"
			"mov %%eax, %1\n\t"
...
static inline __attribute__((always_inline)) uint64_t diff_in_cycles(unsigned high1, unsigned low1,
...
static inline __attribute__((unused)) double diff_in_second(struct timespec t1, struct timespec t2)
{
